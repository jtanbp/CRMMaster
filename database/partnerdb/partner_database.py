    ...
    with conn.cursor() as cur:
        if exclude_id:
            cur.execute(
                "SELECT 1 "
                "FROM partner "
                "WHERE partner_name = %s AND partner_id <> %s AND deleted_at IS NULL",
            ...
            cur.execute(
                "SELECT 1 "
                "FROM partner "
                "WHERE partner_name = %s AND deleted_at IS NULL",
                (partner_name,)
            )
