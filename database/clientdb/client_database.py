...
def edit_client(conn, client_data:dict):
    ...
    try:
        ...
        conn.rollback()
        QMessageBox.critical(None,'DB Error', f'⚠️ Failed to edit client:\n{e}')
...
def client_name_exists(conn, client_name, exclude_id=None):
    ...
    with conn.cursor() as cur:
        if exclude_id:
            cur.execute(
                "SELECT 1 FROM client WHERE client_name = %s AND client_id <> %s AND deleted_at IS NULL",
                (client_name, exclude_id)
            )
        else:
            cur.execute(
                "SELECT 1 FROM client WHERE client_name = %s AND deleted_at IS NULL",
                (client_name,)
            )
